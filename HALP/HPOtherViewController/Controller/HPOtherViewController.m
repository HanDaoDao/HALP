...
-(void)setupView{
    ...
    _button.backgroundColor = hpRGBHex(0xE6E6FA);
    _button.layer.cornerRadius = 20;
    _button.layer.masksToBounds = YES;
    [_button setTitle:@"帮其他忙" forState:UIControlStateNormal];
    [_button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [_button setTitleColor:hpRGBHex(0xF8F8FF) forState:UIControlStateHighlighted];
...
-(void)buttonCompleted{
    HPOtherOrderViewController *otherOderVC = [[HPOtherOrderViewController alloc] init];
    otherOderVC.hidesBottomBarWhenPushed = YES;
    [self.navigationController pushViewController:otherOderVC animated:YES];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSString *string1 ;
    string1 = [[NSString alloc] initWithFormat:@"标题：%@", order.otherContent.title];
//    string2 = [[NSString alloc] initWithFormat:@"送至：%@",order.content.sendTo];
    
    cell.labelOne.text = string1;
    cell.labelTwo.text = order.otherContent.content;
    ...
    [bquery findObjectsInBackgroundWithBlock:^(NSArray *array, NSError *error) {
        ...
        if (error) {
            ...
            for (BmobObject *obj in array) {
                if ([[[obj objectForKey:@"orderType"] objectForKey:@"dataType"] intValue] == 0) {
